// TTC (TrueType Collection) → 单个 TTF/OTF 提取
//
// 因为 opentype.js 不支持 TTC 格式（签名 'ttcf'），我们手动解析 TTC，
// 将其中一个字体重建为标准的 SFNT 文件（TTF/OTF）。
//
// SFNT header 结构:
//   sfntVersion:    4 bytes
//   numTables:      2 bytes
...
/**
 * 从 TTC 文件中提取单个字体，返回其字节数组。
 * @param ttcPath TTC 文件路径
 * @param fontIndex 提取第几个字体（默认 0，建议用 0）
 * @returns 一个可被 opentype.js parse 的 Uint8Array（TTF 或 OTF）
 */
export function extractFontFromTtc(
  ...
  try {
    ...
    p += 2;
    // searchRange/entrySelector/rangeShift 我们重算，直接跳过
    p += 6;

    // 4. 读取每个表的目录：tag | checkSum | offset | length
...
/**
 * 加载字体：自动识别 TTC / TTF / OTF，返回 opentype.js 可用的字节数组。
 */
