...
// 实现SecureRandom类（Node.js环境）
/**
 * 安全随机数生成器
 * @Author ystarry 2023-08-10
 * @Description 安全随机数生成器，提供加密安全的随机数生成方法
 */
export class SecureRandom {
  ...
  // 生成随机整数
  // nextInt():        [0, 2^32)
  // nextInt(bound):    [0, bound)   均匀无偏
  nextInt(bound?: number): number {
    if (bound === undefined) return this.nextUint32();
    if (bound <= 0) throw new RangeError('bound must be positive');
    if (bound === 1) return 0;

    // 若 bound 是 2 的幂：直接位掩码取低位即可，均匀且高效
    if ((bound & -bound) === bound) {
      return this.nextUint32() & (bound - 1);
    }
    // 拒绝采样：消除 2^32 / bound 的非整除带来的尾部偏置
    // 即若 bits 落在 [2^32 - (2^32 % bound), 2^32) 则重新抽取
    // 用 `bits / bound !== Math.floor(bits / bound)` 的安全写法：
    //   bits - (bits % bound) + (bound - 1) == bits + (bound - 1 - (bits % bound))
    //   当它 >= 2^32 时说明 bits 落在尾部（高位溢出）。由于 JS 的 number 能存
    //   下 32 位整数及其和，这里直接与 0x100000000 比较即可。
  ...
  // 生成 [0, 1) 区间的随机双精度浮点数
  nextDouble(): number {
    // 等价于 Java 的 nextDouble：((next(26) << 27) + next(27)) / (1L << 53)
    // 取两个独立的 32 位随机值，分别保留 26 位和 27 位，
    // 然后合并为一个 53 位整数再除以 2^53。
