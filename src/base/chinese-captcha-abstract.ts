...
/**
 * 中文验证码抽象类：默认使用楷体，生成 4 个随机汉字。 / Chinese captcha abstract class: uses KaiTi by default, generates 4 random Chinese characters.
 *
 * 常用汉字表 DELTA 直接以字符串字面量保存，保持与 Java 版内容一致。 / Common Chinese character table DELTA is saved directly as string literal, consistent with the Java version.
 * `alphaHan()` 从中等概率取字。 / `alphaHan()` picks characters with equal probability.
 */
export abstract class ChineseCaptchaAbstract extends Captcha {
  /**
   * 常用汉字字符表（与 Java 版一致）。 / Common Chinese character table (consistent with Java version).
   * 去掉了原 Java 版中的重复 / 容易混淆字符后得到的字表。 / Character table obtained after removing duplicates / easily confused characters from the original Java version.
   */
  ...
  constructor() {
    super();
    // 楷体 28 号字 —— 如果当前环境无 TTF 字体，则退回 system default / KaiTi size 28 —— fall back to system default if no TTF font is available in current environment
  ...
  /**
   * 生成 4 个随机汉字作为验证码。 / Generate 4 random Chinese characters as captcha.
   */
  ...
  /**
   * 返回随机汉字（等概率从 DELTA 取字）。 / Return random Chinese character (picked from DELTA with equal probability).
   * 暴露为 public static，便于测试/扩展。 / Exposed as public static for easy testing/extension.
   */
